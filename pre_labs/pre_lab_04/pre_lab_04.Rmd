...
```{r}
maryland_ppp_with_naics <- maryland_ppp %>% left_join(by="naics_codes")
```
...
**Answer**
Can't join on `x$zip` x `y$zip` because of incompatible types. One zip is categorized as a character data while the other is categorized as a number data.
```{r}
...
**Answer**
No. NA results pop up where ZIP codes are incongruent; some have 9 digits while others have 5 digits.
...
**Answer**
The average loan amount is \$55546.84 and the median loan amount is \$19949.
...
**Answer**
The average loan amount is \$89194.67 and the median loan amount is \$20833.
...
**Answer**
The average loan amount for rural, PNHB populations is \$44815.22 and the median loan amount is \$19064.57.
The average loan amount for urban, PNHB populations is \$55584.05 and the median loan amount is \$19950.00.
...
The average loan amount for rural, non-Hispanic white populations is \$74463.32 and the median loan amount is \$20833.
The average loan amount for urban, non-Hispanic white populations is |$92029.39 and the median loan amount is \$20833.
