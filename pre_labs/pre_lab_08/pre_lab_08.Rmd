...
```{r echo=FALSE}
census_api_key("cd3fd4748e5debb7e9ca66e72e7af850740cb3d4", install=TRUE)
...
**Answer**
Nevada, with a percent change of 35.14.
...
**Answer**
Howard County, with a median income of 121160.
```{r}
...
**Answer**
No — the margin of error for each county's median household income could reverse the order of these counties on the list.

Is the difference between Howard and everyone else significant?
**Answer**
Yes. Even with Howard's MOE, the county is still comfortably atop this list of median household incomes.
...
**Answer**
Somerset County and Caroline County. For Caroline County, it is possible that the estimate is inaccurate based on the MOE — the max or min of 2010 and max of 2019 would show an increase in population for Caroline. This decreases confidence in asserting that Caroline had a decline.
```{r}
md10 %>%
...
**Answer**
I ran the code for these tasks multiple times but am not getting a data frame with the 20 zip codes with the largest amounts, instead I'm getting md_zips_with_acs and top_zips which each have 547 entries. Perhaps I'm misinterpreting the question. I also added code to find which zips in md_zips_with_acs had estimates of under 50000 just in case that is what we're expected to do, and got the following zip codes: 21810, 21653, 21718, 21624, 21522, 21672, 21626, 21675, 21920. Hopefully that is what you're looking for.
...
top_zips_under_fifty_k <- md_zips_with_acs%>%
  filter(total<50000)%>%
     arrange(desc(estimate))

view(top_zips_under_fifty_k)    
```
