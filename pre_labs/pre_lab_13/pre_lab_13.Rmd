...
**Answer**
A 5 column data set with a state_name column, who the state voted for in 2016, the percentage voting for Trump, percentage of businesses that received PPP loans, and the PPP amount per employee.
```{r}
...
**Answer**
A slightly positive correlation between how many businesses received PPP loans and the percentage vote for Trump.
```{r}
...
**Task** Run the following code. Describe the plot. What do you see?
**Answer**
A weaker relationship between PPP per employee and Trump's vote share.
```{r}
...
**Answer**
A larger correlation coefficient of ~.52 for businesses that received PPP loans, and a smaller coefficient of ~.22 for PPP loan amounts per employee.
```{r}
reveal_data %>%
...
**Answer**
A number of variables, including a p-value and correlation coefficient. The p-value is quite low.
```{r}
