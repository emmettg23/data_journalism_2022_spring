...
**Answer**
Dataframe with tweets organized by username, message content, date tweeted, and information about the politician and their district.
```{r}
...
**Answer**
Prints out the word list with words in quotations.
```{r}
...
**Answer**
Creates a new dataframe with all unique words in the original dataframe's tweet contents.
```{r}
...
**Answer**
Lots of short, common, "junky" words that do not indicate anything significant.
...
**Answer**
Yes, now we have a list of pandemic-related vernacular that excludes the "stop words" seen in the previous data frame. An interesting top word is "relief," which is directly related to pandemic aid policy and is a little more niche then the other, generally descriptive pandemic words like "covid" and "pandemic."
```{r}
unique_words %>%
...
**Answer**
There is a greater presence of "vaccine" and "vaccines" in 2021 than 2020, and less mention of "crisis."
```{r}
unique_words_2020 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
Some interesting inclusions, such as "town hall" and "relief package," which are a little less directly related to pandemic and covid cases.
```{r}
covid_tweets %>%
  filter(created < '2021-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
More two-word vaccine chatter, simlilar to the previous one-word results.
```{r}
covid_tweets %>%
  filter(created >= '2021-01-01') %>%
  unnest_tokens(bigram, content, token = "ngrams", n = 2) %>%
...
**Task** Run the following code and describe the results.
**Answer**
Democrats have more mentions of protective measures for COVID-19, whereas Republicans have more general pandemic phrases and the notable presence of "american people."
```{r}
covid_tweets %>%
...
**Task** Run the following code and describe the results.
**Answer**
More mentions of "joe biden," "illegal immigrants," and "covid positive" for Republican candidates in 2021, less mentions of trump.
```{r}
covid_tweets %>%
...
**Answer**
Positive sentiment of "trump" seems misleading, especially knowing that Democrats account for a large portion of "trump" mentions. 
```{r}
