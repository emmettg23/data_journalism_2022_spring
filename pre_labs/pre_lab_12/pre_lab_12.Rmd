...
**Answer**
A large dataframe with 11,397 entries tracking vaccinations per county, per day, and including more specific variables within those vaccinations, such as whether it was the first dose, second dose, and what the cumulative vaccination totals are. 
```{r}
...
**Answer**
An NA row with 284 total entries.
...
**Answer**
Rows of entries from a county described as "NA" and suspiciously round numbers for cumulative dose totals at interesting times, very early during the vaccination rollout.
...
#! in front of code does opposite of function.
```
...
**Answer**
This dataframe displays the dates of total full-vaccinations and where the county ranked on that day, compared to the totals of other counties.
```{r}
...
**Answer**
A peak in May/April 2021 that decreases somewhat steadily until December 2021, where there is a slight uptick.
```{r fig.width=9}
ggplot() +
...
**Answer**
Gradually increasing lines with a big jump around 01-2022.
```{r}
...
**Task** Run the following code and describe what the resulting chart shows
**Answer**
The jump in COVID-19 cases around 01-2022 is much less drastic, and the overall increase is more gradual starting 07-2020.
```{r}
...
**Answer**
Number of COVID cases on 04-18-2022 by Maryland counties.
```{r}
ggplot() +
