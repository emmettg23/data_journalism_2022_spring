...
**Answer**
A long, .csv-looking file (I know it's a .shp, but the commas and columns are slightly similar) that lists geographical features like latitude and longitude, and also address-based information like city, state, and zip.
```{r}
...
**Answer**
A list of hospitals in Maryland with their corresponding geographical features and information about the hospital itself, i.e. what care it provides and whether it's open or closed.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Summary counts of each 'type' of hospital.
```{r}
md_hospitals %>%
  group_by(TYPE) %>%
  summarise(
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A list of the 50 "General Acute Care hospitals" with corresponding .shp file variables.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A plot with points that represent hospitals and form a vague outline of Maryland.
```{r}
md_hospitals %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A snapshot of all U.S. counties and corresponding geographic data.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A 24-row dataframe with MD counties and corresponding geometry.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A lovely map of Maryland with outlines of its counties, although the Chesapeake Bay is filled in.
```{r}
md_counties %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The same map, but with hospital points.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The hospital points now have colors that correspond with the number of beds each hospital contains.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A .shp file with information on all of Maryland's "General Acute Care" hospitals.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Dataframe with tallies of hospital beds for each MD jurisdiction.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Data frame with MD counties and population estimates.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Similar data frame, but with geometry data!
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A data frame with county name and total beds.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A .shp file with population estimates and county geometry.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Similar data frame, but with total beds.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Data frame with calculations of total beds and beds per 100,000 residents.
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
  mutate(total_beds = replace_na(total_beds,0)) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A choropleth map of Maryland that displays counties in terms of the number of hospital beds available per 100,000 residents. 
```{r}
ggplot() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The beds per 100k variable is now displayed in color blocks rather than on a blended scale.
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
...
**Answer**
Two counties on the Eastern Shore lack hospital beds — because they lack hospitals — and Baltimore City and Allegany County have the highest share of hospital beds per 100,000 residents.
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
