...
**Answer**
The first line states: "Warning: One or more parsing issues, see `problems()` for details"
...
Tough to make determination without denominator to contextualize data; i.e. how many Subways are in Maryland?.
...
It assures that the same records with forgiveness_amount as NA are the records with forgiveness_date as NA.
...
(Two draws for PPP; creating gaps between August 2020 and January 2021, and the drop-off in June 2021. Program ended on May 31, 2021. Data is weird but explainable by the administration of the program.)
```{r}
ppp_maryland_loans %>%
...
\$10000000	and \$6. The largest loan amount is not suspicious to me, because that is the max loan amount offered by the PPP. Businesses will always try to seek the max amount of free (or close to free) government money. The smallest loan amount, however, is suspicious to me. Why go through the hassle of filing for a PPP loan if you are only getting \$6 out of it?
```{r}
...
Yes, they all match for both businesses.
...
The ID field starts with a number, the name field is in all caps, and there is a space instead of an underscore in the street address field.
...
The error is: "Error: Problem with `summarise()` column `total_amount`.
ℹ `total_amount = sum(amount)`.
x invalid 'type' (character) of argument"

This must mean that one of the fields we are trying to summarize is a character datatype rather than a number datatype. Therefore, you cannot determine a total.
...
One set of duplicates; STUART JONES LLC.
...
Removes extraneous digits in ZIP field.
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
