...
**Answer**
This page provides a history of the Bureau of Labor Statistic's NAICS codes and charts that describe the sectors, divisions, and codes assigned to each industry.
...
**Answer**
Columns and lists that format the html structure for the overlying webpage.
...
**Answer**
A description of the first couple of lines of the source html page.
...
**Answer**
The environment window highlights the entirety of the table and shows the source code for its creation.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
A table with headings and text tags that formulate the NAICS Sectors table.
### Task 9: Load image
**Task** Run the following code to display an image showing what you should see on the web page.
```{r, echo=FALSE}
...
**Answer**
R Studio displays all of the tables located within the html document and separates them into tibbles.
```{r}
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
A list of the tables on the html page with descriptions of each table's dimensions.
...
**Answer**
Descriptions of the data types for each column and a list of values within the tables.
### Task 14: Load image
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
A table listing NAICS sectors with their corresponding descriptions.
### Task 16: Load image
...
**Answer**
The output is a tibble with the sector of each NAICS code and the corresponding description. 21 rows and 2 columns.
```{r}
...
**Answer**
This code drops the last row in the table, which was a source line for the table's information.
```{r}
# Read in all html from table, store all tables on page as nested list of dataframes.
naics_industry <- naics_url %>%
  read_html() %>%
...
**Answer**
A tibble with two columns for the sector and descriptions of the primary NAICS codes, minus the 21st row, a source tag.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
Employment information for the utilities sector of NAICS.
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer**
The first few lines of the html code that forms the cataloged web page.
```{r}
...
**Answer**
The underlying code for the employment information table of the utilities sector.
```{r}
...
**Answer**
A wacky data frame with mismatching columns and values.
```{r}
...
**Answer**
A much cleaner data frame with one row and two columns, one for employment info and the other for the # of employees in November 2021.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
Employment information and tables for the Mining, Quarrying, and Oil and Gas Extraction industry, similar to the same tables we viewed in the previous industry.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
The same type of page as described before but for the Construction industry.
...
**Answer**
A value of [1] for each of the sectors listed in the code below, which are in quotations. 
```{r}
...
**Answer**
Nothing appears below the codeblock, but a data frame titled "list_of_sectors" forms in the environment table on the right.
```{r}
...
**Answer**
A list of each industry with a value of [1] to the left of the industry title.
```{r}
...
for (sector in list_of_sectors) {
  print(sector)
  #I need to make a url for this sector
  
  #I need to scrape that url into html and save it
  
  #I need to find a specific table in that saved html
  
  #I need to extract one row (the second row) from that table
...
**Answer**
An error message that declares "object 'sector_list' not found," due to the circumstances listed above (the code calls for the wrong object).
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**
A list of the NAICS sectors entered in the below code block.
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**
An error message indicating that the object 'sector_name' is not in our code.
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer**
A list of all numbers between 1 and 10.
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer**
20 rows with the columns: 'sector', 'description', 'sector_url', and 'sector_xpath_id'.
```{r}
...
**Answer**
Counts the number of rows in our naics_industry data frame.
```{r}
nrow(naics_industry)
```
...
**Answer**
Similar to the earlier code of 1-10, this lists values 1-20, which correspond with the number of naics_industry rows.
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
...
**Answer**
A tibble with 1 row and 2 columns, which are 'data_series' and 'nov_2021'.
```{r}
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer**
A tibble with naics codes and sector descriptions, 2 columns and 20 rows.
```{r}
# Define url of page we want to scrape
...
**Answer**
A tibble with columns for NAICS code: sector, description, and a url for each individual sector's web page.
```{r}
...
**Answer**
A tibble with four columns for NAICS code sector, description, a url for each individual sector's employment web page and the tag for the employment table.
...
**Answer**
Same as the previous tibble, minus the "Public Administration" sector. 
```{r}
...
**Answer**
19, 4x1 tables for each NAICS industry and its url + xpath.
```{r}
...
**Answer**
A list of 19 rows with url and xpath information.
```{r}
...
**Answer**
19 data frames with funky columns and row values, all of which include employment statistics.
```{r}
...
**Answer**
19 data frames with much more organized data. Each table has 1 row and 6 columns, the columns display employment stats for 4 months.
```{r}
...
**Answer**
19 data frames with the same contents from the data frames before, plus values for sector, description, sector_url, and sector_xpath_id.
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer**
19 data frames that only includes columns and values for sector, NAICS title, and nov_2021 employment statistics.
```{r}
...
**Answer**
A combined data frame with columns for sector, NAICS title, and nov_2021 employment statistics for 19 industries.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
Information on what constitutes the "Mining" sector and tables with employment statistics within it.
### Task 37: Load image
**Task** Run the following code to display an image showing what you should see on the web page.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
This web page lacks the employment information that is listed under the mining sector. This could be because there is a lack of employer and establishment surveys in this industry, which BLS relies on to form these data tables.
### Task 39: Load image
**Task** Run the following code to display an image showing what you should see on the web page.
...
**Answer**
The agriculture row now has a value of 'N/A' listed under its nov_2021_employees column.
```{r}
