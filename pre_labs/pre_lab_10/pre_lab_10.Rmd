...
https://datawrapper.dwcdn.net/tkZ9C/1/
...
The code below organizes all PPP loans by the county that the recipient is based in and summarizes the total number of loans in each county.
...
**Answer** Describe what the code below is doing and the output.
This code rids the dataframe of the records from counties outside of Maryland, like York County, Pennsylvania, which all have less than 6 total loans.
...
There is a new column that combines the state_code and county_code to form the fips_code.
```{r}
...
Our fips dataframe now has our county ppp loans total from the md_counties dataframe.
```{r}
...
The map is zoomed in to include only Maryland.
...
**Answer** Describe the changes from the previous map
Now when you hover the cursor over a county, you see the county name, not the fips code, and the number of ppp loans is listed in 0,000 format.
...
https://datawrapper.dwcdn.net/ELqOE/1/
