...
**Answer**
This code first narrows down the data by county, it then calculates the total loans for each county, and lastly arranges the top 10 in descending order.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
A cluttered bar chart with the top 10 county PPP loan recipients in Maryland and their total loans. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Answer**
The top 10 Maryland county PPP loan totals in ascending order.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The axes have been inverted, with counties on the y and total PPP loans on the x.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
theme_minimal, which creates a less cluttered background for the bar chart.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Answer**
A light blue background has been added to the canvas.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
We now have titles for our axes.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
Total number of loans per date approved.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
A line graph with loan totals on the y-axis and date approved on the x-axis.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
A crowded x-axis with dates and months layered on top of one another.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
A much cleaner graph, with date labels nicely separated from one another.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
