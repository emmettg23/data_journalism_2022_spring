...
ppp_wv_loans_by_date %>%
  ...
  geom_text(aes(x=date_approved, y=total_loans, label=total_loans), nudge_x = 1.5, nudge_y = 2.25)+
...
#fix state names, rename state column
...
```{r}
state_ppp <- total_pop %>% left_join(state_ppp_all, by=c('state'))%>%
  mutate(ppp_capita= (total_ppp_loans/estimate)*100,000)%>%
  select(state, ppp_capita)%>%
  arrange(desc(ppp_capita)) %>%
  head(10)
```
...
state_ppp %>%
  ggplot() +
  geom_bar(aes(x=reorder(state,ppp_capita), weight=ppp_capita), fill="blue") +
  coord_flip()+
  labs(
...
```
**A3.**  
Based on the scatter plot, it appears that there are two extremes with a high number of values: either all of a loan was forgiven, or none of a loan was forgiven, creating the dense lines along the x-axis and diagonally through the center. A relatively small amount of loans were partially forgiven, and a larger amount was mostly forgiven.
-30-
